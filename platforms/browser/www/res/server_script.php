...
if ($request == 'index') {
	...
	login();
	$sql = "SELECT id, name, min_players, max_players, min_time, max_time
			FROM games
			ORDER BY " . $sort . " ASC";
	$result = $conn->query($sql);
	...
	$count = 0;
	if ($result->num_rows > 0) { while($row = $result->fetch_assoc()) {
		$output[$count]['name'] = (String)utf8_encode($row['name']);
		$output[$count]['p_min'] = utf8_encode($row['min_players']);
		$output[$count]['p_max'] = utf8_encode($row['max_players']);
		$output[$count]['t_min'] = utf8_encode($row['min_time']);
		$output[$count]['t_max'] = utf8_encode($row['max_time']);
		$count++;
		//$output[$row['id']]['quantity'] = utf8_encode($row['quantity']);
		//$output[$row['id']]['teams'] = utf8_encode($row['teams']);
	}}
	echo(json_encode($output));
	logout();
	
} else if ($request == 'udvalg') {
	
	login();
	$sql = "SELECT DISTINCT u.name AS name, u.`fb-group` AS fb, p.first AS first, p.last AS last
			FROM udvalg AS u
	...
	if ($result->num_rows > 0) { while($row = $result->fetch_assoc()) {
		$output[$count]['name'] = utf8_encode($row['name']);
		$output[$count]['fb'] = utf8_encode($row['fb']);
		$output[$count]['leader'] = utf8_encode($row['first']) . ' ' . utf8_encode($row['last']);
		$count++;
	}}
	echo(json_encode($output));
	logout();
	...
	login();
	for ($i = 1; $i < 5; $i++) {
		$sql = "SELECT DISTINCT u.name AS name, a.first AS first, a.last AS last
			FROM `poster` AS u
			LEFT JOIN `alumni_fields` AS f
			ON u.p".$i."=f.nr
			LEFT JOIN `alumni_profile` AS a
			ON f.uid=a.uid
			WHERE LENGTH(u.p".$i.") > 1";
		$result = $conn->query($sql);
		
		if ($result->num_rows > 0) {
			while($row = $result->fetch_assoc()) {
				$output[utf8_encode($row['name'])]['p'.$i] = utf8_encode($row['first']) . ' ' . utf8_encode($row['last']);
			}
		}
	}
	echo(json_encode($output));
